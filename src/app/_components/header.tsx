        ...
        <div className="mx-auto px-4">
          <section className="my-2">
            <div className={`text-xl ${linkClasses}`}>
              <Link href="/" onClick={() => drawerRef.current?.close()}>
                ホーム
              </Link>
            </div>
          </section>
          <section className="my-2">
            <h2 className="text-xl p-1">はじめる</h2>
            <div className={`text-lg ml-4 ${linkClasses}`}>
              <Link href="/select" onClick={() => drawerRef.current?.close()}>
                シラバスを選択する
              </Link>
            </div>
            <div className={`text-lg ml-4 ${linkClasses}`}>
              <Link href="/import" onClick={() => drawerRef.current?.close()}>
                ファイルから読み込む
              </Link>
            </div>
          </section>
